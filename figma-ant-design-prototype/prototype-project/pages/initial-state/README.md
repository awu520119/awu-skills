...
这是 `figma-ant-design-prototype` 的最小可预览模板，适合初始化一个 prototype-design 项目后快速确认发布结果。

## 发布

在包含 `prototype-design` skill 的环境中执行：
...
模板是纯静态 HTML，不依赖 Vite、图片、外部网络资源或其他 prototype-design Skill；发布后可直接替换 `index.html` 和 PRD 内容，继续沿用同一节点结构。
